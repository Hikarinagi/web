...
async function handler(event: H3Event) {
  ...
  const rate = await fetchBackendData(event, '/api/v3/light-novel-volumes/{id}/rates/{rateId}', {
    path: { id, rateId },
  })
  ...
  const [author, sidebar, comments, favorite] = await Promise.all([
    ...
    fetchBackendData(event, '/api/v3/comments', {
      query: {
        target_type: 'light_novel_volume_rate',
        target_id: rateId,
        page: 1,
    ...
    fetchBackendData(event, '/api/v3/user/me/favorite/lightnovels/{light_novel_id}', {
      path: { light_novel_id: rate.work_ref.id },
    }).catch(() => null),
  ])
...
export type VolumeRatePageData = Awaited<ReturnType<typeof handler>>
