  ...
  const [list, histogram, selected_filter_labels] = await Promise.all([
    fetchBackendData(event, '/api/v3/galgames', {
      query: {
        ...
        start_periods: state.start_periods.length ? state.start_periods : undefined,
        end_from: state.end_from,
        end_to: state.end_to,
        end_periods: state.end_periods.length ? state.end_periods : undefined,
        ...
        // dev_status 显式传入时以后端语义为准,覆盖 include_dev 的默认排除
        include_dev: state.dev_status.length ? undefined : state.include_dev || undefined,
        dev_status: state.dev_status.length ? state.dev_status : undefined,
      },
    }),
