...
async function handler(event: H3Event) {
  ...
  const rate = await fetchBackendData(event, '/api/v3/light-novels/{id}/rates/{rateId}', {
    path: { id, rateId },
  })
...
export type LightNovelRatePageData = Awaited<ReturnType<typeof handler>>
