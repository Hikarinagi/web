...
async function handler(event: H3Event) {
  ...
  const rate = await fetchBackendData(event, '/api/v3/mangas/{id}/rates/{rateId}', {
    path: { id, rateId },
  })
...
export type MangaRatePageData = Awaited<ReturnType<typeof handler>>
