<script setup lang="ts">
  import { Alert, Button, Card, Heading, Inline, Progress, Stack, Text } from '@hina-ui/vue'
...
<template>
      ...
      <Card class="pointer-events-auto w-full max-w-sm">
        <Stack gap="lg" align="center" class="text-center">
          ...
          <Inline v-if="showActions" gap="sm" justify="center">
            ...
            <AuthGateButton v-if="error" variant="ghost" tone="neutral" @click="emit('report')">
  ...
  <Inline justify="center" class="pointer-events-none absolute inset-x-0 top-0 z-40 p-4 pt-20">
    ...
    >
      {{ notice?.message }}
      <template #actions>
        <AuthGateButton size="sm" variant="ghost" tone="neutral" @click="emit('report')">
          <template #icon><Flag /></template>
          报告问题
        </AuthGateButton>
      </template>
