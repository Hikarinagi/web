<script setup lang="ts">
  ...
  defineOptions({ name: 'LightNovelRateDimensions' })
    ...
    <AnimatePresence>
      ...
      >
        <div class="flex flex-col gap-3 pt-3">
          <FormItem
            v-for="d in LIGHT_NOVEL_RATE_DIMENSIONS"
            :key="d.key"
            v-slot="{ field }"
            :name="d.key"
          >
            <div class="flex items-center gap-3">
              <span class="w-14 shrink-0 text-[13px] text-surface-700 dark:text-surface-300">
                {{ d.label }}
              </span>
              <Slider
