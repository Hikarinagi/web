<script setup lang="ts">
  ...
  defineOptions({ name: 'LightNovelBrowseBunkoPopover' })
  ...
  const label = computed(() =>
    props.modelValue != null ? (filter?.entityLabel('bunko', props.modelValue) ?? '文库') : '文库',
  )
  ...
  async function fetchList() {
    ...
    loading.value = true
    try {
      const rows = await hikariRequest('/api/v3/light-novels/bunko', {
  ...
  function select(row: { id: number; name: string } | null) {
    if (row) filter?.remember('bunko', [row])
...
<template>
  ...
  <Popover ref="pop" :pt="{ content: { class: 'p-0!' }, root: { class: 'popover-no-arrow' } }">
    <div class="flex w-72 max-w-[calc(100vw-2rem)] flex-col p-2">
      <InputText v-model="query" placeholder="搜索文库…" size="small" fluid />
