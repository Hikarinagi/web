<script setup lang="ts">
  ...
  const MODES = [
    ...
    },
    {
      ...
      subject: '同意授权的用户',
      scopes: 'catalog:*，外加 user:read、status:*、collection:*',
    },
  ]
</script>
      ...
      <table class="w-full min-w-max text-left text-sm whitespace-nowrap">
        <thead class="text-xs text-muted-color">
          <tr class="border-b border-surface">
            <th class="py-2 pr-6 font-normal">令牌</th>
            <th class="py-2 pr-6 font-normal">取法</th>
            <th class="py-2 pr-6 font-normal">代表</th>
        ...
        <tbody>
          ...
          >
            <td class="py-2.5 pr-6 font-medium text-color">{{ mode.token }}</td>
            <td class="py-2.5 pr-6 font-mono text-xs text-muted-color">{{ mode.grant }}</td>
            <td class="py-2.5 pr-6 text-muted-color">{{ mode.subject }}</td>
