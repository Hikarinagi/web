<script setup lang="ts">
  ...
  defineOptions({ name: 'MangaBrowseCard' })
  const props = defineProps<{ item: MangasBrowsePageData['list']['items'][number] }>()
...
<template>
  <NuxtLink :to="`/mangas/${item.id}`" class="group flex min-w-0 flex-col gap-2">
    <div
