<script setup lang="ts">
  ...
  import type { GalgameRateStats } from '~/features/galgame/rate'

  defineOptions({ name: 'GalgameRatesStatsProgress' })
  const props = defineProps<{ counts: GalgameRateStats['status_counts']; played: number }>()
  ...
  const ROWS: {
    ...
    { key: 'completed', label: '通关', icon: CircleCheck, primary: true },
    { key: 'going', label: '在玩', icon: Play },
    { key: 'on_hold', label: '搁置', icon: PauseCircle },
    { key: 'dropped', label: '弃坑', icon: CircleX },
  ]
