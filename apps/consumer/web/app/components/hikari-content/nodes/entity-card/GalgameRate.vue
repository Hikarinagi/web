<script setup lang="ts">
  ...
  defineOptions({ name: 'HikariContentNodesEntityCardGalgameRate' })

  const props = defineProps<{ node: EditorNode }>()
  const summaries = useContentSummaries()

  const id = computed(() => {
    const v = props.node.attrs?.galgame_rate_id
    return typeof v === 'number' && v > 0 ? v : null
  })
  const summary = computed(() =>
    id.value !== null ? (summaries.value.galgame_rates.get(id.value) ?? null) : null,
  )
  const href = computed(() =>
    summary.value ? rateWorkHref('galgame_rate', summary.value.galgame_id) : null,
  )
  const title = computed(() => summary.value?.galgame_title ?? '未命名作品')
  const cover = computed(() => summary.value?.galgame_cover ?? null)
  ...
  const nsfw = computed(() => summary.value?.galgame_nsfw ?? false)
    ...
    <div class="relative z-1 flex items-center gap-3.5">
      ...
      <div class="flex min-w-0 flex-1 flex-col gap-1">
        ...
        >
          ...
          <span class="shrink-0">{{ rater ? '的评分 · Galgame 评分' : 'Galgame 评分' }}</span>
        </span>
        <p
          v-if="excerpt"
          :class="[
