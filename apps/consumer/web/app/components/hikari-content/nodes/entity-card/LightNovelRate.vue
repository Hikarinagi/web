<script setup lang="ts">
  ...
  defineOptions({ name: 'HikariContentNodesEntityCardLightNovelRate' })

  const props = defineProps<{ node: EditorNode }>()
  const summaries = useContentSummaries()

  const id = computed(() => {
    const v = props.node.attrs?.light_novel_rate_id
    return typeof v === 'number' && v > 0 ? v : null
  })
  const summary = computed(() =>
    id.value !== null ? (summaries.value.light_novel_rates.get(id.value) ?? null) : null,
  )
  const href = computed(() =>
    summary.value ? rateWorkHref('light_novel_rate', summary.value.light_novel_id) : null,
  )
  const title = computed(() => summary.value?.light_novel_title ?? '未命名作品')
  const cover = computed(() => summary.value?.light_novel_cover ?? null)
  ...
  const nsfw = computed(() => summary.value?.light_novel_nsfw ?? false)
...
<template>
  <HikariContentNodesEntityCardContainer
    v-if="!blocked"
    :id="id"
    type="light_novel_rate"
    id-attr="data-light-novel-rate-id"
    :href="href"
  >
    ...
    <div class="relative z-1 flex items-center gap-3.5">
      ...
      <div class="flex min-w-0 flex-1 flex-col gap-1">
        ...
        >
          ...
          <span class="shrink-0">{{ rater ? '的评分 · 轻小说评分' : '轻小说评分' }}</span>
        </span>
        <p
          v-if="excerpt"
          :class="[
